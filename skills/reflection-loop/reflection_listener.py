...
def poll_for_completions(interval: int = 10):
    """Poll task board for completed tasks and generate reflections."""
    completed_tasks = set()
    
    print(f"[reflection] Starting polling mode (every {interval}s)...")
    print(f"[reflection] Watching for tasks moving to 'done' status...")
    
    while True:
        try:
            result = _call("GET", "/tasks")
            if result and isinstance(result, dict) and "tasks" in result:
                for task in result["tasks"]:
                    ...
                    # Check if task just completed and not already processed
                    if status == "done" and task_id not in completed_tasks:
                        # Skip reflection tasks themselves
                        if "reflection" in tags:
                            completed_tasks.add(task_id)
                            continue
                        
                        # Skip tasks with no owner (orphaned)
                        if not task.get("owner"):
                            completed_tasks.add(task_id)
                            continue
                        
                        print(f"[reflection] Detected completed task: {task_id} by {task.get('owner')}")
                        process_task_completed({"task": task})
                        completed_tasks.add(task_id)
            
            time.sleep(interval)
            
        except Exception as e:
            print(f"[reflection] Polling error: {e}")
            time.sleep(interval)

def start_reflection_listener(relay_url: str = None, agent_id: str = None, use_sse: bool = False):
    """Entry point to start the reflection listener.
    
    Args:
        relay_url: Relay server URL
        agent_id: This agent's ID  
        use_sse: If True, use SSE streaming. If False, use polling (more reliable).
    """
    ...
    print(f"[reflection] Mode: {'SSE streaming' if use_sse else 'polling (every 10s)'}")
